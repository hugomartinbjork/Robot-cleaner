...
class LIUVacuumEnvironment(Environment):
    ...
    def percept(self, agent):
        return Percept(
            {
                "home": agent.location[0] == 1 and agent.location[1] == 1,
    ...
    def execute_action(self, agent, action):
        ...
        if (
            action == ACTION_SUCK
            and self.world[agent.location[0]][agent.location[1]] == ENV_DIRTY
        ):
        ...
        if action == ACTION_FORWARD:
            new_location = (
                agent.location[0] + agent.facing[0],
                agent.location[1] + agent.facing[1],
            )
        ...
        # Additionally, compute sign of direction along new "active" axis compared to sign of direction along previous axis
            ...
            agent.facing = (
                agent.facing[1],
                -agent.facing[0] if agent.facing[0] != 0 else agent.facing[0],
            )
        elif action == ACTION_TURN_RIGHT:
            agent.facing = (
                -agent.facing[1] if agent.facing[1] != 0 else agent.facing[1],
                agent.facing[0],
            )
    ...
    def randomize_world(self, seed=None):
        ...
        self.world = [
            [
                ...
                or y == 0
                or y == self.env_y - 1
                or (randfunc() < self.wall_bias and not (x == 1 and y == 1))
                else ENV_DIRTY
