...
class Lab1:
    ...
    def __init__(self):
        ...
        self.grid_dims_getter = self.create_selection_menu(
            ...
            *[
                (str(size[1]) + "x" + str(size[0]), size)
                for size in [(5, 5), (10, 10), (15, 15), (20, 20), (5, 10), (10, 5)]
            ]
        )
    ...
    def refresh(self):
        ...
        if (
            self.agent.location != self.marked_agent_pos
            or self.agent.facing != self.marked_agent_rot
        ):
            self.draw_agent()
    ...
    def make_env_frame_canvas(self):
        ...
        if self.grid_frame is None:
            self.grid_frame = Canvas(
                self.host_frame,
                width=(CANVAS_BLOCK_SIZE + CANVAS_BLOCK_OFFSET) * width,
                height=(CANVAS_BLOCK_SIZE + CANVAS_BLOCK_OFFSET) * height,
            )
        else:
        ...
        for x in range(0, width):
            for y in range(0, height):
                ...
                grid[x][y] = self.grid_frame.create_rectangle(
        ...
        if self.grid is None:
            ...
            for y in range(height - 1, -1, -1):
                ...
                row_frame = (
                    ...
                    if rel_y >= previous_height
                    else self.grid[0][previous_height - 1 - rel_y].master
                )
                for x in range(width - 1, -1, -1):
                    rel_x = width - 1 - x
                    grid[x][y] = (
                        self.grid[previous_width - 1 - rel_x][
                            previous_height - 1 - rel_y
                        ]
                        if rel_x < previous_width and rel_y < previous_height
                        else make_button(x, y, row_frame)
                    )
    ...
    def draw_agent(self):
        if USE_CANVAS == 1:
            agent_x = (
                self.agent.location[0] * CANVAS_BLOCK_SIZE
                + CANVAS_BLOCK_OFFSET
                + CANVAS_BLOCK_SIZE / 2
            )
            agent_y = (
                self.agent.location[1] * CANVAS_BLOCK_SIZE
                + CANVAS_BLOCK_OFFSET
                + CANVAS_BLOCK_SIZE / 2
            )
            agent_obj = self.grid_frame.find_withtag("agent")
            if not agent_obj:
                self.grid_frame.create_image(
                    agent_x,
                    agent_y,
                ...
                self.grid_frame.itemconfig(
                    agent_obj, image=self.agent_img[self.agent.facing]
                )
    ...
    def grid_click_callback(self, x, y):
        ...
        if x != 0 and x != w - 1 and y != 0 and y != h - 1:
            self.vacuum_env.world[x][y] = (
                ENV_DIRTY
                if current == ENV_WALL or (x == 1 and y == 1 and current == ENV_CLEAN)
                else ENV_CLEAN
                if current == ENV_DIRTY
                else ENV_WALL
            )
            self.refresh_tile(x, y)
    ...
    def create_selection_menu(
        ...
        *opts,
        ...
        def on_select(*args):
            ...
            if (
                selection_active.get() != selection_previous.get()
